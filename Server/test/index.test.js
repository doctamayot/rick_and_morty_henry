...
describe("Test de RUTAS", () => {
  ...
  describe("GET /rickandmorty/character/:id", () => {
    ...
    it("Si hay un error responde con status: 500", async () => {
      await agent.get("/rickandmorty/character/1234345").expect(500);
    });
  });
  describe("GET /rickandmorty/login", () => {
    it("Si le pasa email y password obtengo el objeto access:true", async () => {
      const response = await agent.get(
        "/rickandmorty/login?email=doctamayot@hotmail.com&password=123456"
      );
      const query = response.body.access;
      expect(query).toBeTruthy();
    });
    it("Si le pasa email incorrecto o password incorecta obtengo el objeto access:false", async () => {
      const response = await agent.get(
        "/rickandmorty/login?email=doctamayott@hotmail.com&password=123456"
      );
      const query = response.body.access;
